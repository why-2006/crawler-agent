...
PAGE_ANALYSIS_PROMPT_TEMPLATE = """你正在爬取网站以提取以下数据："{data_description}"

## 当前进度
- 已抓取页数：{pages_crawled} / {max_pages}
- 最大深度：{max_depth}
- URL 队列中等待抓取的 URL 数：{pending_count}
...
### 任务1：发现值得爬取的链接
从上面的链接列表中选出可能包含目标数据的链接。优先选择：
- 链接文本或 URL 路径暗示包含目标数据（如视频详情、用户主页、列表页）
- 排除：登录、注册、关于、设置、帮助、协议等无关页面
- 排除：URL 高度重复的页面（选择 1-2 个代表即可）

### 任务2：提取目标数据
从页面文本中提取与目标数据描述匹配的结构化信息。

以 JSON 格式返回：
{{"selected_urls": ["url1", "url2"], "extracted_items": [{{"字段1": "值1"}}], "reason": "分析理由"}}
如果没有找到相关数据或链接，对应字段返回空数组。
只返回 JSON，不要包含其他文字。"""
